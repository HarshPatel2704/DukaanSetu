...
const ShopkeeperOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const res = await axios.get('https://dukaansetu-backend.onrender.com/api/orders/shopkeeper', {
  ...
  const updateStatus = async (orderId, currentStatus) => {
    ...
    try {
      ...
      const res = await axios.put(
        `https://dukaansetu-backend.onrender.com/api/orders/${orderId}/status`,
        { status: nextStatus },
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    <>
      <style>{`
        ...
        :root {
          ...
          --shadow-hover: 0 10px 32px rgba(24,20,48,0.11);
        }
        ...
        .ds-so-title span { color: var(--indigo); }
        ...
        .ds-order-card.pending   { border-left: 3px solid #FF6B6B; }
        ...
        .ds-status-badge.pending   { background: rgba(245,158,11,0.1); color: #D64545; border: 1px solid rgba(245,158,11,0.25); }
        ...
        .ds-toggle-btn.to-pending {
          border: 1.5px solid rgba(245,158,11,0.35); background: transparent; color: #D64545;
        }
        .ds-toggle-btn.to-pending:hover { background: rgba(245,158,11,0.07); border-color: #D64545; }
        ...
        .ds-avatar {
          ...
          box-shadow: 0 3px 10px var(--indigo-glow);
        }
        .ds-customer-name { font-weight: 600; font-size: 0.9rem; color: var(--deep); }

        .ds-address-block {
          background: #FCFBF9; border: 1px solid var(--border-soft);
        ...
        .ds-inner-table td {
          ...
          color: var(--deep); border-bottom: 1px solid #F0EDFA; vertical-align: middle;
        }
        ...
        .ds-inner-table tbody tr:hover { background: #FCFBF9; }

        .ds-item-name { display: flex; align-items: center; gap: 0.45rem; }
        .ds-item-dot { width: 6px; height: 6px; border-radius: 50%; background: linear-gradient(135deg, var(--indigo), var(--indigo-light)); flex-shrink: 0; }
        .ds-qty-chip {
        ...
        .ds-empty-icon {
          width: 60px; height: 60px; border-radius: 18px; background: #F0EDFA;
          ...
          {orders.length > 0 && (
            <div className="ds-summary-row">
              <div className="ds-summary-pill">
                <span className="ds-summary-pill-dot" style={{ background: 'var(--indigo)' }} />
              ...
              <div className="ds-summary-pill">
                <span className="ds-summary-pill-dot" style={{ background: '#FF6B6B' }} />
                <span style={{ color: 'var(--muted)' }}>Pending</span>
                <span className="ds-summary-pill-val" style={{ color: '#D64545' }}>{pending}</span>
              </div>
            </div>
          )}
          ...
          {orders.length === 0 ? (
            ...
            <>
              ...
              {orders.filter(o => o.status !== 'delivered').length > 0 && (
                <>
                  <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.4rem', color: '#D64545', margin: '2rem 0 1rem' }}>Pending Orders</h3>
